...
WITH ctePreDrugTarget(drug_exposure_id, person_id, ingredient_concept_id, drug_exposure_start_date, days_supply, drug_exposure_end_date)   AS (SELECT d.drug_exposure_id
    ...
    FROM vid_consign.drug_exposure d
        JOIN vid_consign.concept_ancestor ca ON ca.descendant_concept_id = d.drug_concept_id
        JOIN vid_consign.concept c ON ca.ancestor_concept_id = c.concept_id
...
ANALYZE tmp_de
;
INSERT INTO vid_consign.drug_era(drug_era_id,person_id, drug_concept_id, drug_era_start_date, drug_era_end_date, drug_exposure_count, gap_days)
SELECT * FROM tmp_de;