---
title: "OMOP ETL Implementation part 3: CONSIGN"
...
1. Set parameter name of schema that contains CDMv5 instance (@TARGET_CDMV5, @TARGET_CDMV5_SCHEMA).

2. Run this script through SqlRender to produce a script that will work in your source dialect.

3. Run the script produced by SQL Render on your target RDBDMS.

<RDBMS> File Instructions

1. This script will hold a number of placeholders for your CDM V4 and CDM V5 database/schema. In order to make this file work in your environment, you should plan to do a global "FIND AND REPLACE" on this file to fill in the file with values that pertain to your environment. The following are the tokens you should use when doing your "FIND AND REPLACE" operation:
    
     postgres
     postgres.vid_consign

## condition_era
    
```{r}
# sql instruction with the database and schema names replaced---------------
sql_condition_era <- "
...
FROM postgres2.vid_consign.CONDITION_OCCURRENCE co;
...
INSERT INTO postgres2.vid_consign.condition_era (
    condition_era_id
    ,person_id
    ,condition_concept_id
...
```{r}
# make the .sql file--------------------------------------------------------
script_postgresql_condition_era <- SqlRender::translate(sql_condition_era,
                                              targetDialect = "postgresql")

cat(script_postgresql_condition_era)
...
```{r}
# sql instruction with the database and schema names replaced---------------
sql_drug_era <- "if object_id('tempdb..#tmp_de', 'U') is not null drop table #tmp_de;
...
```{r}
# make the .sql file--------------------------------------------------------
script_postgresql_drug_era <- SqlRender::translate(sql_drug_era,
                                              targetDialect = "postgresql")

cat(script_postgresql_drug_era)
