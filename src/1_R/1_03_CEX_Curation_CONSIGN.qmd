...
table_sample <- fread(file.path(path_origenes_consign, 
            "03_CEX", "ext2", "V99",
            "SIS_31692_SD2270_CEX_SUBSANACION.sql.log.enc.txt"),
...
clean_cex_2018_2021 <- function(x, ...){
  ...
  select(sip = num_sip,
         ...
         especialidad_cod = cod_serv_hom,
         especialidad_desc = desc_serv_hom,
         d1_cod = cod_diag1,
         d1_desc = desc_diag1,
         d2_cod = cod_diag2,
         d2_desc = desc_diag2,
...
```{r}
# load and clean base--------------------------------------------------------
cex_0 <- clean_cex_2018_2021(file.path(path_origenes_consign, 
            "03_CEX", "ext2", "V99",
            "SIS_31692_SD2270_CEX_SUBSANACION.sql.log.enc.txt")) 
...
codigos_cex <- cex_1 |>
  select(d1_cod, d1_desc, d2_cod, d2_desc, d3_cod, d3_desc, d4_cod, d4_desc) |>
  distinct() |> 
  transmute(
    d1_cod_desc = str_c(d1_cod, d1_desc, sep = "|"),
    d2_cod_desc = str_c(d2_cod, d2_desc, sep = "|"),
    d3_cod_desc = str_c(d3_cod, d3_desc, sep = "|"),
    d4_cod_desc = str_c(d4_cod, d4_desc, sep = "|")) |> 
  pivot_longer(d1_cod_desc:d4_cod_desc,
               values_to = c("diag_cod_desc")) |> 
  select(-name) |> 
  distinct() |> 
  separate(diag_cod_desc, into = c("diag_cod", "diag_desc"), sep ="\\|")
...
```{r}
# Detect NANDA---------------------------------------------------------------
nanda1 <- codigos_cex |> distinct(diag_cod) |> 
  arrange(diag_cod) |> 
  ...
  pull(diag_cod)
nanda2 <- codigos_cex |> distinct(diag_cod) |> 
  arrange(diag_cod) |>
...
cie9_1 <- codigos_cex |> distinct(diag_cod) |> 
  arrange(diag_cod) |> 
...
cie10_1 <- codigos_cex |> distinct(diag_cod) |> 
  ...
  filter(nchar(diag_cod) == 3) |> 
  pull(diag_cod)
cie10_2 <- codigos_cex |> distinct(diag_cod) |> 
  arrange(diag_cod) |>
  filter(str_starts(diag_cod, "V") == FALSE) |>
  filter(str_starts(diag_cod, "E") == FALSE) |>
  filter(str_starts(diag_cod, "\\d") == FALSE) |> 
  ...
  pull(diag_cod)
# cie10 E
cie10_3 <- codigos_cex |> distinct(diag_cod) |> 
  arrange(diag_cod) |>
  filter(str_starts(diag_cod, "E\\d\\d") == TRUE) |>
  filter(nchar(diag_cod) == 3) |> 
  pull(diag_cod)
cie10_4 <- codigos_cex |> distinct(diag_cod) |> 
  arrange(diag_cod) |>
  filter(str_starts(diag_cod, "E\\d\\d\\.") == TRUE) |> 
  pull(diag_cod)
# cie10 V
cie10_5 <- codigos_cex |> distinct(diag_cod) |> 
  arrange(diag_cod) |>
  filter(str_starts(diag_cod, "V") == TRUE) |>
  filter(nchar(diag_cod) > 7) |> 
  pull(diag_cod)
cie10_6 <- codigos_cex |> distinct(diag_cod) |> 
  arrange(diag_cod) |>
  filter(str_starts(diag_cod, "V") == TRUE) |>
...
```{r}
# create tipo_codigo---------------------------------------------------------
codigos_maestro_cex_0 <- codigos_cex |> 
  tidylog::mutate(
    tipo_codigo = case_when(
...
codigos_maestro_cex_0 |> count(tipo_codigo)

```

```{r}
# fix dubious ICD9CM/ICD10ES codes-----------------------------------------------
codigos_maestro_cex_1 <- codigos_maestro_cex_0 |> 
...
```{r}
# check codes----------------------------------------------------------------
codigos_maestro_cex_1 |> filter(str_starts(diag_cod, "V") == TRUE) |>
  distinct(diag_cod, diag_desc, tipo_codigo) |>
  arrange(diag_cod)
 
codigos_maestro_cex_1 |> count(tipo_codigo)

codigos_maestro_cex_1 |> filter(tipo_codigo == "Others") |> 
  distinct(diag_cod, diag_desc) |>
  arrange(diag_cod)
 
codigos_maestro_cex_1 |> filter(tipo_codigo == "No ref") |>
  distinct(diag_cod, diag_desc)
...
```{r}
# cex codes------------------------------------------------------------------
map_cex_icd9_to_concept_id_0 <- codigos_maestro_cex_1 |>
...
# check
wrong_cex_icd9_codes <- map_cex_icd9_to_concept_id_0 |>
...
wrong_cex_icd9_codes_corrected <- wrong_cex_icd9_codes |> 
...
saveRDS(wrong_cex_icd9_codes_corrected, file.path("Resultados", "icd_issues",
                               "wrong_cex_icd9_codes_corrected.RDS"))
...
#| tbl-cap: "ICD9CM Code Issues in cex Table"
#| label: tbl-cexicd9issues

# table of the wrong codes and the proposed fix------------------------------
wrong_cex_icd9_codes_corrected <- readRDS(file.path("Resultados", "icd_issues",
                               "wrong_cex_icd9_codes_corrected.RDS"))

wrong_cex_icd9_codes_corrected |>
  gt(id = "cexicd9") |>  # need to name the table to apply CSS
...
```{r}
# fix the unmapped codes in the cex table------------------------------------
codigos_maestro_cex_2 <- codigos_maestro_cex_1 |> 
  left_join(wrong_cex_icd9_codes_corrected |> 
...
map_cex_icd9_to_concept_id <- codigos_maestro_cex_2 |>
  filter(tipo_codigo == "ICD9CM") |>
...
```{r}
# cex codes------------------------------------------------------------------
map_cex_icd10_to_concept_id_0 <- codigos_maestro_cex_2 |>
...
# check
wrong_cex_icd10_codes <- map_cex_icd10_to_concept_id_0 |>
  select(diag_cod, concept_id) |> 
  filter(is.na(concept_id)) |>
  select(-concept_id) |> 
  mutate(.before = 1, issue = if_else(str_detect(diag_cod, "\\d?[aA-zZ]"), 
...
```{r}
# check the fix
codigos_maestro_cex_2 |>
  filter(tipo_codigo == "ICD10ES") |> 
  distinct(diag_cod, diag_desc) |>
  filter(diag_cod %in% (wrong_cex_icd10_codes |> pull(diag_cod))) |> 
  arrange(diag_cod)

icd10_concepts |> filter(concept_code == "O36.63")
icd10_concepts |> filter(concept_code == "O41.00")
icd10_concepts |> filter(concept_code == "S39.91")
icd10_concepts |> filter(concept_code == "S82.63")
icd10_concepts |> filter(concept_code == "T74.21")
icd10_concepts |> filter(concept_code == "T81.4")
icd10_concepts |> filter(concept_code == "T81.89")
...
```{r}
# fix the unmapped codes-----------------------------------------------------
wrong_cex_icd10_codes_corrected <- wrong_cex_icd10_codes |> 
  mutate(diag_cod_cleaned = case_when(
  # correct errors
  diag_cod == "O36.63X"  ~ "O36.63",
  diag_cod == "O41.00X"  ~ "O41.00"	,
  diag_cod == "S39.91X"  ~ "S39.91",
  diag_cod == "S82.63X"  ~ "S82.63",
  diag_cod == "T74.21X"  ~ "T74.21",
  diag_cod == "T81.4XX"  ~ "T81.4",
  diag_cod == "T81.89X"  ~ "T81.89",
  T ~ diag_cod
    ))

saveRDS(wrong_cex_icd10_codes_corrected, file.path("Resultados", "icd_issues",
                               "wrong_cex_icd10_codes_corrected.RDS"))
...
#| echo: false
#| tbl-cap-location: top
#| tbl-cap: "ICD10CM Code Issues in cex Table"
#| label: tbl-cexicd10issues

# table of the wrong codes and the proposed fix------------------------------
wrong_cex_icd10_codes_corrected <- readRDS(file.path("Resultados", "icd_issues",
                               "wrong_cex_icd10_codes_corrected.RDS"))

wrong_cex_icd10_codes_corrected |>
  gt(id = "cexicd10") |>  # need to name the table to apply CSS
  fmt_markdown(columns = everything()) |> 
  cols_label(issue = "Issue",
             diag_cod = "Original code",
             diag_cod_cleaned = "Corrected code"
...
```{r}
# fix the unmapped codes in the cex table------------------------------------
codigos_maestro_cex_3 <- codigos_maestro_cex_2 |> 
  left_join(wrong_cex_icd10_codes_corrected |> 
...
```{r}
# map again with the corrected codes-----------------------------------------
map_cex_icd10_to_concept_id <- codigos_maestro_cex_3 |>
  filter(tipo_codigo == "ICD10ES") |>
  mutate(diag_cod = if_else(!is.na(diag_cod_cleaned),
                            diag_cod_cleaned, diag_cod)) |>
  select(-diag_cod_cleaned) |> 
...
# Include tipo_codigo into CEX

```{r}
# add tipo_codigo1,2,3,4 and corrected codes---------------------------------
cex_2 <- cex_1 |> 
  # add tipo_codigo1
  left_join(codigos_maestro_cex_3, 
            by = c("d1_cod" = "diag_cod", "d1_desc" = "diag_desc")) |>
  mutate(d1_cod = if_else(!is.na(diag_cod_cleaned),
                            diag_cod_cleaned, d1_cod)) |> 
  select(-diag_cod_cleaned) |> 
  rename(tipo_codigo1 = tipo_codigo) |>  
  # add tipo_codigo2
  left_join(codigos_maestro_cex_3, 
            by = c("d2_cod" = "diag_cod", "d2_desc" = "diag_desc")) |> 
  mutate(d2_cod = if_else(!is.na(diag_cod_cleaned),
                            diag_cod_cleaned, d2_cod)) |> 
  select(-diag_cod_cleaned) |> 
  rename(tipo_codigo2 = tipo_codigo) |> 
  # add tipo_codigo3
  left_join(codigos_maestro_cex_3, 
            by = c("d3_cod" = "diag_cod", "d3_desc" = "diag_desc")) |> 
  mutate(d3_cod = if_else(!is.na(diag_cod_cleaned),
                            diag_cod_cleaned, d3_cod)) |> 
  select(-diag_cod_cleaned) |> 
  rename(tipo_codigo3 = tipo_codigo) |> 
  # add tipo_codigo4
  left_join(codigos_maestro_cex_3, 
            by = c("d4_cod" = "diag_cod", "d4_desc" = "diag_desc")) |> 
  mutate(d4_cod = if_else(!is.na(diag_cod_cleaned),
                            diag_cod_cleaned, d4_cod)) |> 
  select(-diag_cod_cleaned) |> 
  rename(tipo_codigo4 = tipo_codigo) 

# check
cex_2 |> count(tipo_codigo1)
cex_2 |> count(tipo_codigo2)
cex_2 |> count(tipo_codigo3)
cex_2 |> count(tipo_codigo4)
...
cex <- cex_2 |> 
  ...
  tidylog::mutate(especialidad_desc = enc2utf8(especialidad_desc)) |> 
  tidylog::distinct()
...
# save in SSD
fwrite(cex, append = FALSE, 
       file = file.path("Origin_tables", "cex.csv"))
# save in CERES
fwrite(cex, file.path(procesados, "cex.csv"))
...
unlink(file.path("F:", "CERES2", "CONSIGN_SD2270", "2-SCRIPTS", "R",
                 "1_03_CEX_curation_CONSIGN.qmd"))
file.copy(from = file.path("1_03_CEX_curation_CONSIGN.qmd"),
