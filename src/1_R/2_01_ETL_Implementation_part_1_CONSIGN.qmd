...
gaia_exported_stcm <- fread(file.path("..", "Consign", "Resultados",
                         "gaia_masters", "usagi", 
"gaia_exported_stcm_2024_06_11.csv"))
...
from_gaia_to_drug_exposure <- from_gaia_to_drug_exposure_0 |> 
  ...
  left_join(gaia_exported_stcm_to_join,
            by = c("pres_farma_cod")) |>
    mutate(drug_concept_id = if_else(is.na(target_concept_id),
...
- Tensión arterial diastólica: 3012888 (Diastolic blood pressure). 
- Tensión arterial sistólica: 3004249 (Systolic blood pressure). 
- Talla: 3015514 (Body height –standing) 
- Peso: 3025315 (Body weight) 
- Índice de Masa Corporal: 3038553 (Body mass index (BMI) [Ratio]).
...
from_tests_to_measurement <- tests |> 
  ...
  transmute(
    ...
    measurement_concept_id = case_when(
      descripcion == "Tensión arterial diastólica" ~ 3012888,
      descripcion == "Tensión arterial sistólica" ~ 3004249,
      descripcion == "Talla" ~ 3015514,
      descripcion == "Peso" ~ 3025315,
      descripcion == "Índice de Masa Corporal" ~ 3038553,
      T ~ 0),
    measurement_date = fecha,
    measurement_datetime = fecha,
    measurement_time = fecha,
    measurement_type_concept_id = if_else(
descripcion != "Índice de Masa Corporal", 32836, 32880),
